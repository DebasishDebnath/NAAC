...
function Forms() {
  ...
  const grabTableIndexFind = (input) => {
    ...
    for (let outerIndex = 0; outerIndex < reportOptions.length; outerIndex++) {
      ...
      for (let innerIndex = 0; innerIndex < items.length; innerIndex++) {
        const normalizedItem = items[innerIndex]
          .replace(/\s+/g, "")
          .toLowerCase();
  ...
  const fetchReports = async () => {
    ...
    try {
      console.log(
        `Fetching data from endpoint "${endPointGet}" for table "${selectedForm.backend_table_name}"`
      );
      ...
      if (response?.data && selectedForm?.backend_table_name) {
        ...
        if (tableData) {
          const formattedData = Array.isArray(tableData)
            ...
            : [tableData];
          console.log(
            `Received ${formattedData.length} report(s) for ${selectedForm.tableName}`
          );
